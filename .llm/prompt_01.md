Vamos construir a camada silver de um e-commerce brasileiro no Databricks, como um Lakeflow Declarative Pipeline dentro deste bundle. Use sempre o perfil de CLI "projetodados".

CONTEXTO

Todo o trabalho acontece no catálogo projetodados. Nunca use outro catálogo.
A bronze já existe e é sobrescrita por outra ingestão: projetodados.bronze.vendas, bronze.produtos, bronze.clientes e bronze.preco_competidores.
...
Pipeline serverless, catálogo projetodados, schema padrão silver. Golds publicadas como gold.. Nomes no código sempre schema.tabela, sem catálogo.
...
Crie o notebook testes/testes_qualidade.py (formato source do Databricks, widget catalogo com padrão projetodados). Cada teste é uma consulta que conta linhas com problema; se alguma achar, o notebook falha com AssertionError e mostra uma tabela com o resultado de cada teste. Por enquanto: chaves únicas das 4 silver; receita = quantidade × preco_unitario; vendas de produto não cadastrado abaixo de 1% do total.
